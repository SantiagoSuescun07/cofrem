...
import { TriviaGameDetails, TriviaQuestion } from "@/types/games";
import { updateRanking } from "@/services/games/update-ranking";
import GameInstructions from "./GameInstructions";

interface TriviaGameProps {
  gameDetails: TriviaGameDetails;
  onClose: () => void;
  campaignNid?: number;
}
...
export default function TriviaGame({
  ...
  const questions = gameDetails.field_questions || [];
  const currentQuestion: TriviaQuestion | null = questions[currentQuestionIndex] || null;
  const totalQuestions = questions.length;
  const isGameComplete = currentQuestionIndex >= totalQuestions;

  // Mezclar las respuestas para cada pregunta
  const getShuffledAnswers = (question: TriviaQuestion | null): string[] => {
    if (!question) return [];
    
    return [
      question.field_correct_answer,
      question.field_incorrect_answer_1,
      question.field_incorrect_answer_2,
      question.field_incorrect_answer_3,
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        <div className="bg-white rounded-2xl border-2 border-red-400 shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-red-600 mb-6">No hay preguntas disponibles para esta trivia.</p>
          <button
            onClick={onClose}
          ...
          {isGameComplete ? (
            ...
            <div className="text-center">
              ...
              <div className="flex gap-4 justify-center mt-6">
                <button
                  onClick={handleRetry}
                  className="px-6 py-3 bg-gradient-to-r from-[#2deb79] to-green-600 text-white rounded-full text-lg font-semibold shadow-lg hover:from-green-600 hover:to-green-700 transition-all duration-200 transform hover:scale-105"
                >
                  🔄 Jugar de Nuevo
                </button>
                <button
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-lg font-semibold flex-1">{answer}</span>
                          {showResult && (
